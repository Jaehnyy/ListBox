...
void CListBoxView::OnInitialUpdate()
{
	...
	m_combo1.AddString(_T("사과"));
	m_combo1.AddString(_T("딸기"));
	m_combo1.AddString(_T("파인애플"));
	m_combo1.AddString(_T("복숭아"));
}
...
void CListBoxView::OnBnClickedRight()
{
	...
	if (nIndex != CB_ERR) {
		...
		nIndex=m_combo1.DeleteString(nIndex);
		m_combo1.SetCurSel(nIndex); // 삭제된 다음 위치의 항목이 자동 선택됨
...
void CListBoxView::OnBnClickedLeft()
{
	...
	if (nIndex != CB_ERR) {
		...
		nIndex = m_combo2.DeleteString(nIndex);
		m_combo2.SetCurSel(nIndex); // 삭제된 다음 위치의 항목이 자동 선택됨 
